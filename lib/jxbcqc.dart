import 'dart:convert';
import 'dart:io';

import 'package:crypto/crypto.dart';
import 'package:device_info_plus/device_info_plus.dart';
import 'package:flutter/material.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:uuid/uuid.dart';

import 'cbfvt.dart';
import 'ulbyp.dart';
import 'sqcsn.dart';

///  VUlIqWkcTViEgNsd4EdYxlXUqg
/// kCQ1RSVmkGl
enum ZiJdXXDrbrDKjv { wp0a, ikAy }

class Z8KqNHv7rNLeZ6q01d5p {
  /// xX92uXS2z30Pws85fOX
  static ZiJdXXDrbrDKjv w47oYjX = ZiJdXXDrbrDKjv.ikAy;
}

/// UXIAmyuV rAgXeC1x
class WLTSxolylqtv {
  /// T3bQQH4wFXK1mImtPz0UYeIFIV5
  /// OqhU15NP2gdJ0E0Qpui
  /// 4nsl79SYUrLN09SKp9Osi
  static const String pcgJ22on = 'shixinim';
  // 5h0DIMpecALm7cYOo6QY
  static final String _z0dHBCsOurLB4 = String.fromCharCodes(const <int>[
    119,
    107,
    95,
    105,
    115,
    95,
    104,
    117,
    97,
    119,
    101,
    105,
    95,
    100,
    101,
    118,
    105,
    99,
    101,
  ]);
  // nd93wy98t15iSU53dI5slOGX9
  static const String uDN2XsBkb0 = String.fromEnvironment(
    'SIGN_SECRET',
    defaultValue: 'your-secret-key',
  );

  /// 5yT2S4R1nS5RD
  static const String tHWlbkBeNjt4MA =
      'https://raw.githubusercontent.com/appsupdate/apps/main/android_test.json';
  static const String w3X4IJvQtO =
      'https://raw.githubusercontent.com/appsupdate/apps/main/ios_test.json';

  /// mepvz 8v6u
  /// omXLiN8R6QMXWGidl9l
  static const String idILHsl3NNYd4d =
      'https://raw.githubusercontent.com/appsupdate/apps/main/android_prod.json';
  static const String sY2oUAq2oa =
      'https://raw.githubusercontent.com/appsupdate/apps/main/ios_prod.json';

  /// lIkU4FgIFY
  static String p1fm48ecx() {
    return Z8KqNHv7rNLeZ6q01d5p.w47oYjX == ZiJdXXDrbrDKjv.wp0a
        ? w3X4IJvQtO
        : sY2oUAq2oa;
  }

  // dBC0av6SSje6uhgSsFwT45A32
  // vJ uTTTrv3 owY595KvEV3pL8jxL
  // gL1OyHmhg00BKnDSz90
  // qINBCzE JtR I8yt L wgyg
  // B3nF85j7J3eCljy7l
  // 8WYFsF9lIhAyvNXflWw3v3zR
  // VhcDpMMSY9OSrvL6CXmVgBKCwhKr
  // PBufZwx1D7qq
  // 0 gh7cw Kqr
  // w6z7LD5NcSXa5YTirR1b KCWaw
  // XnJ2TlaznjFUmXk
  // JxHltssuqD33M1 6uFSsieXT

  // CvV dELbVTEnroF
  // MTI5CGonjErNp4Ru3SUm TvcVyb
  // UiCaBIk0iw eZndQs3i
  // QND0AVvmcukgu27qyy1fZwL8RHpS
  // sKIpKGpq6FKD7zffE
  // Ke qi3zDAyahcEKqTU8DVILQ
  // EBYZjvvruOH NIcYnRl8KdU
  // BflB9T1v 4rqEjnuuOAJJgeT
  // 7reh5GemvuIG

  // mc9lv7znJFOgk
  // RM1EKDwwxdLLXNoaTmp5b

  // FqXxX9gsuZRHnEoU5Ha
  // dCLARLT3b4QmPGwOmsl3DzJ
  // CIsDlpVzeFEdsJMfpOqex0CTt
  // MZlANx5cMg4O0Uf
  // lpH4ZGDP4ABxI0XaJ6Q

  // fZLgikP1fzOcmkgL
  // xwW8aLFwlA ubXJu1En
  // R7AB7OWR5i
  // ZuOv6phltFCx4s
  // hycmWHHnvK4idNdIB3SKJNZZEEt
  // zI7pTei1KWKTg
  // i7Y1m1S6kLs
  // dpiVvyZDjrYanZRYW
  /// fS7NyNnaKLIQ7rLaQjm
  static String aMfgs5uxck3p2() {
    return Z8KqNHv7rNLeZ6q01d5p.w47oYjX == ZiJdXXDrbrDKjv.wp0a
        ? tHWlbkBeNjt4MA
        : idILHsl3NNYd4d;
    if (false) {
      print(String.fromCharCodes(const <int>[121, 73, 118, 81, 113]));
    }
  }
}

/// lNJKa9dkkmf
class P6yedlHkdEt {
  static String folu035 = '';
  static String htlLMQyMCF = '';
  static String vO3C0eDAqPX3HNiS =
      Z8KqNHv7rNLeZ6q01d5p.w47oYjX == ZiJdXXDrbrDKjv.wp0a
      ? String.fromCharCodes(const <int>[
          104,
          116,
          116,
          112,
          115,
          58,
          47,
          47,
          105,
          109,
          46,
          109,
          121,
          105,
          109,
          46,
          100,
          101,
          118,
          47,
        ])
      : String.fromCharCodes(const <int>[
          104,
          116,
          116,
          112,
          115,
          58,
          47,
          47,
          105,
          109,
          46,
          109,
          121,
          106,
          109,
          121,
          46,
          99,
          111,
          109,
          47,
        ]);

  // LI0L2NaZpUpes0DLguGvc
  // D6o23TyNsQGBQID0V8
  static String rqbTuPqvAm0h2c =
      Z8KqNHv7rNLeZ6q01d5p.w47oYjX == ZiJdXXDrbrDKjv.wp0a
      ? String.fromCharCodes(const <int>[
          104,
          116,
          116,
          112,
          115,
          58,
          47,
          47,
          97,
          112,
          105,
          45,
          116,
          101,
          115,
          116,
          46,
          109,
          121,
          105,
          109,
          46,
          100,
          101,
          118,
        ])
      : String.fromCharCodes(const <int>[
          104,
          116,
          116,
          112,
          115,
          58,
          47,
          47,
          99,
          100,
          110,
          46,
          115,
          104,
          105,
          120,
          105,
          110,
          97,
          114,
          105,
          46,
          99,
          111,
          109,
        ]);

  // YAcan6O8M5YmWijFc
  static String eYWmqXhVF0ueBwDhzNtjJP =
      Z8KqNHv7rNLeZ6q01d5p.w47oYjX == ZiJdXXDrbrDKjv.wp0a
      ? String.fromCharCodes(const <int>[
          104,
          116,
          116,
          112,
          115,
          58,
          47,
          47,
          97,
          112,
          105,
          45,
          116,
          101,
          115,
          116,
          46,
          109,
          121,
          105,
          109,
          46,
          100,
          101,
          118,
        ])
      : String.fromCharCodes(const <int>[
          104,
          116,
          116,
          112,
          115,
          58,
          47,
          47,
          99,
          100,
          110,
          46,
          115,
          104,
          105,
          120,
          105,
          110,
          97,
          114,
          105,
          46,
          99,
          111,
          109,
        ]);

  // PZRYST4BvTc
  static final List<String> _r44M91FnvXRa1G6LtK29V = [
    String.fromCharCodes(const <int>[
      104,
      116,
      116,
      112,
      115,
      58,
      47,
      47,
      97,
      112,
      105,
      45,
      116,
      101,
      115,
      116,
      50,
      46,
      109,
      121,
      105,
      109,
      46,
      100,
      101,
      118,
    ]),
  ];

  static final List<String> _gOeqgSr7eRAeTo1WuIRmGE = [
    String.fromCharCodes(const <int>[
      104,
      116,
      116,
      112,
      58,
      47,
      47,
      105,
      109,
      103,
      46,
      97,
      112,
      112,
      100,
      110,
      115,
      46,
      115,
      105,
      116,
      101,
    ]),
    String.fromCharCodes(const <int>[
      104,
      116,
      116,
      112,
      115,
      58,
      47,
      47,
      105,
      109,
      103,
      46,
      97,
      112,
      112,
      100,
      110,
      115,
      46,
      111,
      110,
      108,
      105,
      110,
      101,
    ]),
    String.fromCharCodes(const <int>[
      104,
      116,
      116,
      112,
      115,
      58,
      47,
      47,
      105,
      109,
      103,
      46,
      97,
      112,
      112,
      100,
      110,
      115,
      46,
      115,
      112,
      97,
      99,
      101,
    ]),
    String.fromCharCodes(const <int>[
      104,
      116,
      116,
      112,
      115,
      58,
      47,
      47,
      105,
      109,
      103,
      46,
      97,
      112,
      112,
      100,
      110,
      115,
      46,
      115,
      116,
      111,
      114,
      101,
    ]),
    String.fromCharCodes(const <int>[
      104,
      116,
      116,
      112,
      115,
      58,
      47,
      47,
      105,
      109,
      103,
      46,
      97,
      112,
      112,
      100,
      110,
      115,
      46,
      119,
      111,
      114,
      107,
    ]),
    String.fromCharCodes(const <int>[
      104,
      116,
      116,
      112,
      115,
      58,
      47,
      47,
      105,
      109,
      103,
      46,
      97,
      112,
      112,
      100,
      110,
      115,
      46,
      120,
      121,
      122,
    ]),
    String.fromCharCodes(const <int>[
      104,
      116,
      116,
      112,
      115,
      58,
      47,
      47,
      99,
      100,
      110,
      46,
      115,
      104,
      105,
      120,
      105,
      110,
      97,
      114,
      105,
      46,
      99,
      111,
      109,
    ]),
  ];

  static List<String> get s3HZWMmiUSlLeWkzoJ =>
      Z8KqNHv7rNLeZ6q01d5p.w47oYjX == ZiJdXXDrbrDKjv.wp0a
      ? _r44M91FnvXRa1G6LtK29V
      : _gOeqgSr7eRAeTo1WuIRmGE;

  // OnlsSovurNgpSBMWB
  static const String rmAoULhvaJ5h9 = 'https://www.shixinari.com/privacy.html';
  static const String sa4MzHK5aYpLi = 'https://www.shixinari.com/service.html';

  /// VhUZNV8FBI7dKMJL
  /// ECyXYzPR6r66eXLILOtP
  static String ijdVhfZ2RfmQM8() {
    return '$rqbTuPqvAm0h2c/v1/';
    if (1 == 2) {
      var var_nilTH = String.fromCharCodes(const <int>[85, 117, 99, 117, 108]);
    }
  }

  static void pLIT7LUr8BlrziaH6TN8(String apiURL) {
    folu035 = '$apiURL/v1/';
    htlLMQyMCF = '$apiURL/web/';
  }

  static String dM33rYrj05EW(String uid) {
    return '${fnvYpLxvFodGj9ly5()}users/$uid/avatar';
  }

  /// uM4MfbWGD7U6k
  /// iuLuAqyR0siJdECKAwBbq mgg
  static Future<void> hkuI5eXGPAJkfisq22tR() async {
    try {
      {
        var var_nViCA = String.fromCharCodes(const <int>[101, 82, 74, 97, 118]);
      }
      final prefs = await SharedPreferences.getInstance();
      final savedLine =
          prefs.getString(
            String.fromCharCodes(const <int>[
              115,
              97,
              118,
              101,
              100,
              95,
              108,
              105,
              110,
              101,
            ]),
          ) ??
          '';

      if (savedLine.isNotEmpty) {
        rqbTuPqvAm0h2c = savedLine;
        S0jlNL.aNRrP(
          '🔵 WKApiConfig: Line server loaded from storage: $rqbTuPqvAm0h2c',
        );
      } else {
        if (1 == 2) {
          var var_rtscE = String.fromCharCodes(const <int>[
            86,
            83,
            76,
            122,
            107,
          ]);
        }
        S0jlNL.aNRrP('🔵 WKApiConfig: Using original default: $rqbTuPqvAm0h2c');
      }
    } catch (e) {
      S0jlNL.xPrk5(
        String.fromCharCodes(const <int>[
          55357,
          56629,
          32,
          87,
          75,
          65,
          112,
          105,
          67,
          111,
          110,
          102,
          105,
          103,
          58,
          32,
          70,
          97,
          105,
          108,
          101,
          100,
          32,
          116,
          111,
          32,
          108,
          111,
          97,
          100,
          32,
          108,
          105,
          110,
          101,
          32,
          115,
          101,
          114,
          118,
          101,
          114,
          44,
          32,
          117,
          115,
          105,
          110,
          103,
          32,
          100,
          101,
          102,
          97,
          117,
          108,
          116,
        ]),
        error: e,
      );
    }
    if (false) {
      print(String.fromCharCodes(const <int>[115, 89, 82, 110, 71]));
    }
  }

  static String hXzcryjZUnr(String groupId) {
    return '${fnvYpLxvFodGj9ly5()}groups/$groupId/avatar';
  }

  /// 8o9g3QlMkhgg
  /// rj8aKzyy2uRnBuEdeyrtu5InqBKI6
  static String fnvYpLxvFodGj9ly5() {
    return folu035.isNotEmpty ? folu035 : '$rqbTuPqvAm0h2c/v1/';
    if (1 == 2) {
      var var_CJGVV = String.fromCharCodes(const <int>[109, 118, 112, 113, 74]);
    }
  }

  static void dedBzWZlt4l(String apiURL) {
    folu035 = '$apiURL/v1/';
    htlLMQyMCF = '$apiURL/web/';
  }

  /// Z8AcSdZI199CWhXhYG Vy9Iyiych
  /// LcDRgJIwdqV eXblSiW4
  static bool hUz2cLAsxtUAZj9w(String url) {
    if (url.isEmpty) return false;

    final normalized = url.split(String.fromCharCodes(const <int>[63])).first;
    final lastDot = normalized.lastIndexOf(
      String.fromCharCodes(const <int>[46]),
    );
    if (lastDot == -1 || lastDot == normalized.length - 1) {
      return true;
      if (false) {
        print(String.fromCharCodes(const <int>[111, 99, 83, 98, 109]));
      }
    }

    final ext = normalized.substring(lastDot + 1).toLowerCase();
    final allowed = {
      String.fromCharCodes(const <int>[106, 112, 103]),
      String.fromCharCodes(const <int>[106, 112, 101, 103]),
      String.fromCharCodes(const <int>[112, 110, 103]),
      String.fromCharCodes(const <int>[103, 105, 102]),
      String.fromCharCodes(const <int>[119, 101, 98, 112]),
      String.fromCharCodes(const <int>[98, 109, 112]),
      String.fromCharCodes(const <int>[104, 101, 105, 99]),
      String.fromCharCodes(const <int>[104, 101, 105, 102]),
    };

    return allowed.contains(ext);
  }

  static String jzMyuEhRqt(String url) {
    if (url.isEmpty ||
        url.startsWith(String.fromCharCodes(const <int>[104, 116, 116, 112])) ||
        url.startsWith(String.fromCharCodes(const <int>[72, 84, 84, 80]))) {
      return url;
    } else {
      return fnvYpLxvFodGj9ly5() + url;
      if (1 == 2) {
        var var_mNjZM = String.fromCharCodes(const <int>[118, 111, 87, 77, 83]);
      }
    }
  }

  static String sm3EGJ3ZBSbr7GVj(String url) {
    if (url.isEmpty) return '';

    if (url.startsWith(String.fromCharCodes(const <int>[104, 116, 116, 112])) ||
        url.startsWith(String.fromCharCodes(const <int>[72, 84, 84, 80]))) {
      return url.contains(String.fromCharCodes(const <int>[107, 101, 121]))
          ? url
          : '$url?key=${DateTime.now().microsecondsSinceEpoch}';
      if (false) {
        print(String.fromCharCodes(const <int>[105, 88, 110, 83, 85]));
      }
    }

    return fnvYpLxvFodGj9ly5() + url;
  }
}

/// 1g LK7JCx0MFzDJ
class AIdp6jCI1sa {
  static const String lnufu36bDXxpnCQ = 'refresh_contacts';

  // GcHw8iic9lkpMv
  // H2blTFf9199ElWSH8CB83rCay
  static const String fW7WuB4LOHiNaCK = 'wk_new_msg_notification';
  // OjDMXio4hE3VtWA
  static const String yzn1lNISRefSHdzJl78IY4xH =
      'wk_new_msg_notification_sound_only';
  // Y2D9fueNe ru4FVcgFVJmN8Acldg
  static const String kZcH0diwZNrYzsbEllaSkjadKH =
      'wk_new_msg_notification_vibrate_only';
  // hegNMr26ga
  static const String nELRYVievYEIwFqBHYJxD = 'wk_new_msg_notification_silent';

  static const String rlMGB8fBVLuwW7o = 'wk_new_rtc_notification';

  // pWmBqGUsG8ZXo
  static const String fPbiZ4KiaMeht = 'api_base_url';
  static const String qhHWuwPUSKR = 'user_info';
  static const String zYsQADf1uMMI = 'app_config';
  static const String fXkVeWNk = 'wk_token';
  static const String sd1R6myq5onm = 'wk_auth_token';
  static const String mzWt0C1lCz = 'wk_im_token';
  static const String x6eQ6x = 'wk_uid';
  static const String tdp3oDWo3ZT = 'wk_name';
  static const String b1cQCI8tzWlfaorgK = 'keyboardHeight';
  static const String jRxfAkqi3GRH = 'app_module';
  static const String pkH5Ln1NO3JzWyHc8iMMlZFAG0c =
      'group_chat_interval_seconds';
  static const String mfDzu8iW7rJzmq2aWfnS61P = 'group_chat_last_sent_map';

  // DnsRls1k0t7lnv4hm5OJfiuvxx
  static const String mCs8pmFDkQepaFB4nFPnCo = 'show_agreement_dialog';

  // 4X6f7SNAtRe1Qui2ntn tj4kf
  static const String aWkOfu70LJpPBNaA5 = 'last_login_phone';
  static const String xsuH4nEobaWj40oM = 'last_login_zone';

  // 7 zSkuTJ9mf Vh
  // fYhlvk4UNgBZ 015r
  static const String fjgtRNbGS5eoDXNVbN = 'has_ever_logged_in';

  // VGZRdtqcNK1veaDA
  static const String vcPKayXTngBSii = 'media_switch';

  static const String elN8a8dgSIVVTqc7L8kd4Lprzu3ExC7 =
      'is_initialized_permission_media';

  static Future<int> dKsIPfk0uK9lBDoxsuDRmD7LSir() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      return prefs.getInt(pkH5Ln1NO3JzWyHc8iMMlZFAG0c) ?? 0;
    } catch (_) {
      return 0;
      if (1 == 2) {
        var var_BcAgt = String.fromCharCodes(const <int>[
          110,
          67,
          110,
          104,
          66,
        ]);
      }
    }
  }

  // fFpZwEuSruKPveOSHAzy
  static Future<void> xK5ieWCN13xXWEwGJ67sxYmEp48(int value) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      if (1 == 2) {
        var var_ZCnBX = String.fromCharCodes(const <int>[
          121,
          77,
          98,
          113,
          114,
        ]);
      }
      await prefs.setInt(pkH5Ln1NO3JzWyHc8iMMlZFAG0c, value);
    } catch (_) {}
  }

  /// zKDQmJwCKs6s7x1hwu7uR0vteOr
  /// WryQDEN ZoJeMvl1dNtlcUkvNO
  static Future<int> bBn0jq3ZL2EHNd() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      return prefs.getInt(vcPKayXTngBSii) ?? 0;
    } catch (_) {
      return 0;
    }
    if (false) {
      print(String.fromCharCodes(const <int>[71, 120, 100, 69, 111]));
    }
  }

  static const String e6lGbCLJ1R2 = 'wk_device_id';

  // jEjpAHUDy5I84SWeiESlbOU3nN
  static String? _tUIRChr7;

  /// 66tUf3cLLwDsMmCNrX9
  /// LRRU2K79wV XJVQgBg7cIaaSqlwI
  static Future<void> ywUQILZ8sfpOnsjflZ() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      String? savedDeviceId = prefs.getString(e6lGbCLJ1R2);

      if (savedDeviceId == null || savedDeviceId.isEmpty) {
        // rxbtxvIJOcLxU
        savedDeviceId = await _oWafDY0M3LsUaFcB();
        await prefs.setString(e6lGbCLJ1R2, savedDeviceId);
        S0jlNL.aNRrP('🔑 WKConstants: Generated new device ID: $savedDeviceId');
      } else {
        S0jlNL.aNRrP(
          '🔑 WKConstants: Loaded device ID from storage: $savedDeviceId',
        );
        if (1 == 2) {
          var var_KYnlJ = String.fromCharCodes(const <int>[
            101,
            117,
            104,
            114,
            117,
          ]);
        }
      }

      _tUIRChr7 = savedDeviceId;
    } catch (e) {
      S0jlNL.xPrk5(
        String.fromCharCodes(const <int>[
          55357,
          56593,
          32,
          87,
          75,
          67,
          111,
          110,
          115,
          116,
          97,
          110,
          116,
          115,
          58,
          32,
          70,
          97,
          105,
          108,
          101,
          100,
          32,
          116,
          111,
          32,
          105,
          110,
          105,
          116,
          105,
          97,
          108,
          105,
          122,
          101,
          32,
          100,
          101,
          118,
          105,
          99,
          101,
          32,
          73,
          68,
        ]),
        error: e,
      );
      // PYId2Q0URGaDEmz7 G0RlL77
      _tUIRChr7 = await _oWafDY0M3LsUaFcB();
    }
  }

  /// VETyQJ6OtFHhXrnTIbsVSj
  /// UvfN8kIhC7GIzi0
  static Future<void> mx95p4VKOXdQJ() async {
    try {
      if (false) {
        print(String.fromCharCodes(const <int>[70, 110, 80, 119, 83]));
      }
      final prefs = await SharedPreferences.getInstance();
      await prefs.remove(e6lGbCLJ1R2);
      _tUIRChr7 = null;
      S0jlNL.aNRrP(
        String.fromCharCodes(const <int>[
          55357,
          56593,
          32,
          87,
          75,
          67,
          111,
          110,
          115,
          116,
          97,
          110,
          116,
          115,
          58,
          32,
          68,
          101,
          118,
          105,
          99,
          101,
          32,
          73,
          68,
          32,
          99,
          108,
          101,
          97,
          114,
          101,
          100,
        ]),
      );
    } catch (e) {
      S0jlNL.xPrk5(
        String.fromCharCodes(const <int>[
          55357,
          56593,
          32,
          87,
          75,
          67,
          111,
          110,
          115,
          116,
          97,
          110,
          116,
          115,
          58,
          32,
          70,
          97,
          105,
          108,
          101,
          100,
          32,
          116,
          111,
          32,
          99,
          108,
          101,
          97,
          114,
          32,
          100,
          101,
          118,
          105,
          99,
          101,
          32,
          73,
          68,
        ]),
        error: e,
      );
    }
    if (1 == 2) {
      var var_tMTWl = String.fromCharCodes(const <int>[86, 107, 82, 67, 117]);
    }
  }

  /// 6vQ qZPLVrZ fottWX NE7RT
  /// UPhCGcZ9wjqzwrC3YfV7UGP0MevT
  static Future<String> lfPbNJTyl5T() async {
    if (_tUIRChr7 == null || _tUIRChr7!.isEmpty) {
      S0jlNL.irn4Gh7(
        String.fromCharCodes(const <int>[
          9888,
          65039,
          32,
          87,
          75,
          67,
          111,
          110,
          115,
          116,
          97,
          110,
          116,
          115,
          58,
          32,
          68,
          101,
          118,
          105,
          99,
          101,
          32,
          73,
          68,
          32,
          110,
          111,
          116,
          32,
          105,
          110,
          105,
          116,
          105,
          97,
          108,
          105,
          122,
          101,
          100,
          44,
          32,
          103,
          101,
          110,
          101,
          114,
          97,
          116,
          105,
          110,
          103,
          32,
          116,
          101,
          109,
          112,
          111,
          114,
          97,
          114,
          121,
          32,
          73,
          68,
        ]),
      );
      final deviceID = await _oWafDY0M3LsUaFcB();
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(e6lGbCLJ1R2, deviceID);
      S0jlNL.aNRrP('🔑 WKConstants: Generated temporary device ID: $deviceID');
      _tUIRChr7 = deviceID;
    }
    return _tUIRChr7!;
  }

  static Future<String> _oWafDY0M3LsUaFcB() async {
    {
      var var_uATuH = String.fromCharCodes(const <int>[86, 98, 108, 101, 79]);
    }
    try {
      final deviceInfo = DeviceInfoPlugin();

      if (false) {
        print(String.fromCharCodes(const <int>[119, 107, 68, 82, 105]));
      }
      String raw;
      if (Platform.isAndroid) {
        final android = await deviceInfo.androidInfo;
        final androidId = android.id;
        raw = androidId;
      } else if (Platform.isIOS) {
        final ios = await deviceInfo.iosInfo;
        final idfv = ios.identifierForVendor;
        raw = '$idfv';
      } else {
        raw =
            'other-${Platform.operatingSystem}-${Platform.operatingSystemVersion}';
      }
      final bytes = utf8.encode(raw);
      final digest = md5.convert(bytes);
      return digest.toString();
    } catch (e) {
      return const Uuid().v4().toString().replaceAll(
        String.fromCharCodes(const <int>[45]),
        "",
      );
    }
  }
}

/// 2s fZGgUb7
class LTWIND0TK3iW79z {
  static const String hKyQY5u8Md = 'u_10000';
  static const String y6390CDIiq6PZ8vG = 'fileHelper';
  static const String gXasuH4MBUP = 'admin';
  static const String ffTh0b9rqdLjSjTyC = '10000';
  static const String bBhL7GBKtrMCeVe2QBF6F2Z = '20000';

  static bool pcwqSTlYcMBe(String? uid) {
    if (uid == null || uid.isEmpty) return false;
    final normalized = uid.toLowerCase();
    return normalized == hKyQY5u8Md.toLowerCase() ||
        normalized == y6390CDIiq6PZ8vG.toLowerCase() ||
        normalized == gXasuH4MBUP.toLowerCase();
  }
}

/// Lozqakv2xhgoL32tVjXSvf
class On6sYEc {
  static const String qU6GoNx = '时信IM';
  static const String wXXOJJbWM = 'Shixin';
  static const String xKYoPj1lnki = 'com.shixincrmor.litechat';
}

/// 00OaFrxHN50eqjmIx5b
class JnrtSlrn8YhjEis {
  static const int jAz5aeHO9sxYPoWr6 = 6;
  static const int cL6zfqkhfgaqXPk9B = 20;
  static const int po7OVrT5zjWL2kvyol = 11;
  static const int hJlEVvFjJ0WOltqRQK8JDUVl9S = 12;
  static const String aC36AvSbNKiBrTBUfe = '0086';

  /// 3cJEg0qT9yiFcXlr1YXI7rZUH3rji
  static bool mgG26jQoV2q5oKtn0RPyXYizC(String phone) {
    final length = phone.length;
    return length == po7OVrT5zjWL2kvyol || length == hJlEVvFjJ0WOltqRQK8JDUVl9S;
  }
}

/// kNy1pJjyGeIpyUdAn9LW8YF
/// 4HijU9fGLabDAGPcX4B
/// xhJ3yG NbLZHK0cnfNH2cJCK
class BnCjvEbin6YbS {
  // wUPeQXwluH bSRTOApx1aTfSsW
  static String ckGGifrjzhLsAyO(BuildContext context) =>
      AppLocalizations.of(context)?.dlk3tctvwuo ??
      String.fromCharCodes(const <int>[
        78,
        105,
        99,
        107,
        110,
        97,
        109,
        101,
        32,
        99,
        97,
        110,
        110,
        111,
        116,
        32,
        98,
        101,
        32,
        101,
        109,
        112,
        116,
        121,
      ]);
  static BuildContext? get _h677GTE8yi =>
      UfCaTaCPaluKFU7aj.fxIvlmllkUEZ.currentContext;

  // oLIMhLqKSsiGYp7xFw
  static String m5TlF5KEdOS(BuildContext context) =>
      AppLocalizations.of(context)?.yhhdgqx9ndnbb3y ??
      String.fromCharCodes(const <int>[
        76,
        111,
        103,
        105,
        110,
        32,
        102,
        97,
        105,
        108,
        101,
        100,
      ]);
  static String get tjggYiwM3DLbJYg7 => _h677GTE8yi != null
      ? m5TlF5KEdOS(_h677GTE8yi!)
      : String.fromCharCodes(const <int>[
          76,
          111,
          103,
          105,
          110,
          32,
          102,
          97,
          105,
          108,
          101,
          100,
        ]);
  static String gP6ERTnVuwFB(BuildContext context) =>
      AppLocalizations.of(context)?.xrhuzljapqyawvvgl ??
      String.fromCharCodes(const <int>[
        78,
        101,
        116,
        119,
        111,
        114,
        107,
        32,
        101,
        114,
        114,
        111,
        114,
      ]);
  static String get mcdHRWY9AhTIWMWOJ => _h677GTE8yi != null
      ? zwbHGDqj9cww(_h677GTE8yi!)
      : String.fromCharCodes(const <int>[
          85,
          110,
          107,
          110,
          111,
          119,
          110,
          32,
          101,
          114,
          114,
          111,
          114,
          32,
          111,
          99,
          99,
          117,
          114,
          114,
          101,
          100,
        ]);
  static String nV8aeng2ed(BuildContext context) =>
      AppLocalizations.of(context)?.bkm2hgb3x5izqq ??
      String.fromCharCodes(const <int>[
        73,
        110,
        118,
        97,
        108,
        105,
        100,
        32,
        112,
        104,
        111,
        110,
        101,
        32,
        110,
        117,
        109,
        98,
        101,
        114,
      ]);
  static String wrnkGLkPnQ6sPQVefS(BuildContext context) =>
      AppLocalizations.of(context)?.hh7o5htji2ryac ??
      String.fromCharCodes(const <int>[
        86,
        101,
        114,
        105,
        102,
        105,
        99,
        97,
        116,
        105,
        111,
        110,
        32,
        102,
        97,
        105,
        108,
        101,
        100,
      ]);

  //  EUu1uqpfza9YxB
  static String s1lstRBvmN(BuildContext context) =>
      AppLocalizations.of(context)?.r2nri6bami5dny ??
      String.fromCharCodes(const <int>[
        80,
        97,
        115,
        115,
        119,
        111,
        114,
        100,
        32,
        99,
        97,
        110,
        110,
        111,
        116,
        32,
        98,
        101,
        32,
        101,
        109,
        112,
        116,
        121,
      ]);
  static String get gbPZ4CR7yFHZHzmzDVRIcq6 => _h677GTE8yi != null
      ? wrnkGLkPnQ6sPQVefS(_h677GTE8yi!)
      : String.fromCharCodes(const <int>[
          86,
          101,
          114,
          105,
          102,
          105,
          99,
          97,
          116,
          105,
          111,
          110,
          32,
          102,
          97,
          105,
          108,
          101,
          100,
        ]);
  static String get jjhpMGgIfSiwjB8pk => _h677GTE8yi != null
      ? gP6ERTnVuwFB(_h677GTE8yi!)
      : String.fromCharCodes(const <int>[
          78,
          101,
          116,
          119,
          111,
          114,
          107,
          32,
          101,
          114,
          114,
          111,
          114,
        ]);
  static String zwbHGDqj9cww(BuildContext context) =>
      AppLocalizations.of(context)?.sky6yw7steby8 ??
      String.fromCharCodes(const <int>[
        85,
        110,
        107,
        110,
        111,
        119,
        110,
        32,
        101,
        114,
        114,
        111,
        114,
        32,
        111,
        99,
        99,
        117,
        114,
        114,
        101,
        100,
      ]);
  static String o9oHvdTkyDv(BuildContext context) =>
      AppLocalizations.of(context)?.o3i1goxybahf ??
      String.fromCharCodes(const <int>[
        78,
        97,
        109,
        101,
        32,
        99,
        97,
        110,
        110,
        111,
        116,
        32,
        98,
        101,
        32,
        101,
        109,
        112,
        116,
        121,
      ]);
  static String plQZq2SoB11Ot(BuildContext context) =>
      AppLocalizations.of(context)?.ln1ohsicvhy2ypgb ??
      String.fromCharCodes(const <int>[
        80,
        108,
        101,
        97,
        115,
        101,
        32,
        97,
        103,
        114,
        101,
        101,
        32,
        116,
        111,
        32,
        116,
        104,
        101,
        32,
        116,
        101,
        114,
        109,
        115,
        32,
        97,
        110,
        100,
        32,
        99,
        111,
        110,
        100,
        105,
        116,
        105,
        111,
        110,
        115,
      ]);
  static String get b6v9Zlh8YSoHG2zptuu => _h677GTE8yi != null
      ? rM7bYrbrVbysE9(_h677GTE8yi!)
      : String.fromCharCodes(const <int>[
          82,
          101,
          103,
          105,
          115,
          116,
          114,
          97,
          116,
          105,
          111,
          110,
          32,
          102,
          97,
          105,
          108,
          101,
          100,
        ]);
  static String r7RQw7DZ4AW31g(BuildContext context) =>
      AppLocalizations.of(context)?.negah3q96ekua ??
      String.fromCharCodes(const <int>[
        80,
        97,
        115,
        115,
        119,
        111,
        114,
        100,
        32,
        109,
        117,
        115,
        116,
        32,
        98,
        101,
        32,
        54,
        45,
        50,
        48,
        32,
        99,
        104,
        97,
        114,
        97,
        99,
        116,
        101,
        114,
        115,
      ]);
  static String rM7bYrbrVbysE9(BuildContext context) =>
      AppLocalizations.of(context)?.swsm3rg631m ??
      String.fromCharCodes(const <int>[
        82,
        101,
        103,
        105,
        115,
        116,
        114,
        97,
        116,
        105,
        111,
        110,
        32,
        102,
        97,
        105,
        108,
        101,
        100,
      ]);
}

/// hopfhn6QbQsaG
class GyB0fPUKPyj {
  static const double zuQTHyHDZPhg4n = 16.0;
  static const double hFUsOVEbt9Zt = 8.0;
  static const double cvXZGWrJQdDs = 24.0;
  static const double zRSUu0kgrlcU = 8.0;
  static const double aqfHAIWXubzz = 48.0;
  static const double szQirqv6m4z = 56.0;
}
