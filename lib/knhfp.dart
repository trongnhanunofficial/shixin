...
class MtPB74sreTKqioBbt {
  ...
  void _cvCFfHZPhFxCNG9m7pWCqa7() async {
    ...
    try {
      ...
      for (int attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (_mFSBcjicP0ii != null && cCGarEakAkZkDW.srcObject == null) {
          ...
          S0jlNL.aNRrP(
            '🐥 STREAM DATA CHECK: Successfully set remote stream to renderer (attempt ${attempt + 1})',
            tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
          );
          S0jlNL.aNRrP(
            '🐥 STREAM DATA CHECK: Remote stream has ${_mFSBcjicP0ii!.getVideoTracks().length} video tracks and ${_mFSBcjicP0ii!.getAudioTracks().length} audio tracks',
            tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
          );
          S0jlNL.a7hKJES(
        ...
        if (cCGarEakAkZkDW.srcObject != null) {
          ...
          S0jlNL.aNRrP(
            '🐥 STREAM DATA CHECK: Renderer srcObject has ${videoTracks.length} video tracks, ${audioTracks.length} audio tracks',
            tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
          );
    ...
    { var var_cksbp = String.fromCharCodes(const <int>[111, 102, 106, 101, 114]); }
            S0jlNL.aNRrP(
              '🐥 STREAM DATA CHECK: Video track ${track.id} in renderer - enabled: ${track.enabled}, muted: ${track.muted}',
              tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
            );
          }
  ...
  Future<void> _iW88Q8QIbCGsV1Li0Jl(
    ...
    _nK77UBZ88kngZNxsA5SU1ved.putIfAbsent(
          ...
          S0jlNL.aNRrP(
            '🐥 ICE CANDIDATE: 🟡 Buffered remote ICE (callId=$callId). '
        ...
        try {
          S0jlNL.aNRrP(
            '🐥 ICE CANDIDATE: Adding candidate - ${candidateMap[String.fromCharCodes(const <int>[99, 97, 110, 100, 105, 100, 97, 116, 101])]}',
            tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
          );
  ...
  Future<void> vsYld3BPn1(FTqvfNZE5av3QXqkpPq offer, String fromUid) async {
    ...
    S0jlNL.aNRrP(
      '🐥 CALL ANSWER: Answering call from $fromUid with callId: ${offer.xMPxsX}',
      tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
    );
  ...
  Future<void> _vCpX5cMJ86kxT6P7a8yQ() async {
    ...
    S0jlNL.aNRrP(
      '🐥 ICE CONFIG: Creating peer connection with ${_iFFeH5Dee6PuR4n5.length} ICE servers',
      tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
    );
    for (int i = 0; i < _iFFeH5Dee6PuR4n5.length; i++) {
      final server = _iFFeH5Dee6PuR4n5[i];
      S0jlNL.aNRrP(
        '🐥 ICE CONFIG: Server $i: ${server[String.fromCharCodes(const <int>[117, 114, 108, 115])]} ${server.containsKey(String.fromCharCodes(const <int>[117, 115, 101, 114, 110, 97, 109, 101])) ? String.fromCharCodes(const <int>[40, 119, 105, 116, 104, 32, 97, 117, 116, 104, 41]) : String.fromCharCodes(const <int>[40, 83, 84, 85, 78, 32, 111, 110, 108, 121, 41])}',
        tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
      );
    }
    ...
    _vn!.onIceCandidate = (RTCIceCandidate c) async {
      S0jlNL.aNRrP('🐥 ICE CANDIDATE: Generated candidate - ${c.candidate}', tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]));
    ...
    S0jlNL.a7hKJES(
      ...
      S0jlNL.aNRrP(
        '🐥 STREAM DATA CHECK: Received ${event.streams.length} streams from remote peer',
        tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
      );
      for (int i = 0; i < event.streams.length; i++) {
        final stream = event.streams[i];
        S0jlNL.aNRrP(
          '🐥 STREAM DATA CHECK: Stream $i has ${stream.getVideoTracks().length} video tracks and ${stream.getAudioTracks().length} audio tracks',
          tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
        );
        ...
        for (var videoTrack in stream.getVideoTracks()) {
          S0jlNL.aNRrP(
            '🐥 STREAM DATA CHECK: Video track ${videoTrack.id} enabled: ${videoTrack.enabled}',
            tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
          );
        }
        ...
        for (var audioTrack in stream.getAudioTracks()) {
          S0jlNL.aNRrP(
            '🐥 STREAM DATA CHECK: Audio track ${audioTrack.id} enabled: ${audioTrack.enabled}',
            tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
          );
        }
      }
    ...
    _vn!.onConnectionState = (RTCPeerConnectionState state) {
      ...
      S0jlNL.aNRrP('🐥 CONNECTION STATE: Peer connection state changed to: $state', tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]));
    ...
    _vn!.onIceConnectionState = (RTCIceConnectionState state) {
      S0jlNL.aNRrP('🐥 ICE STATE: ICE connection state changed to: $state', tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]));
    ...
    _vn!.onIceGatheringState = (RTCIceGatheringState state) {
      S0jlNL.aNRrP('🐥 ICE GATHERING: ICE gathering state changed to: $state', tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]));
  ...
  Future<void> _whQwBYLLmvjBFK0SZSw8PZffvXLCX(String callId) async {
    ...
    S0jlNL.aNRrP(
      '🐥 ICE CANDIDATE: Draining ${pending.length} buffered remote ICE candidates for callId=$callId',
      tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
    );
    ...
    { var var_ZOHiM = String.fromCharCodes(const <int>[68, 80, 98, 69, 101]); }
      try {
        ...
        S0jlNL.aNRrP('🐥 ICE CANDIDATE: ✅ Drained candidate added: ${c[String.fromCharCodes(const <int>[99, 97, 110, 100, 105, 100, 97, 116, 101])]}', tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]));
      } catch (e) {
  ...
  Future<void> z8FEENzeu({
    ...
    S0jlNL.aNRrP(
      '🐥 CALL START: Starting call to $peerUid with callId: $_q964EdQ8Xsduh',
      tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
    );
  ...
  void mbAmC61YgCXYwXIMx() {
    ...
    if (_mFSBcjicP0ii != null) {
      ...
      Future.delayed(const Duration(milliseconds: 100)).then((_) {
        ...
        S0jlNL.aNRrP('🐥 FORCE REFRESH: Video renderer now has stream: $hasVideo', tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]));

        if (hasVideo) {
          final videoTracks = cCGarEakAkZkDW.srcObject!.getVideoTracks();
          S0jlNL.aNRrP(
            '🐥 FORCE REFRESH: Renderer now has ${videoTracks.length} video tracks',
            tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
          );
    ...
    S0jlNL.aNRrP(
            ...
            S0jlNL.aNRrP(
              '🐥 TEXTURE CHECK: Renderer textureId: ${cCGarEakAkZkDW.textureId}',
              tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
            );
            S0jlNL.aNRrP('🐥 TEXTURE CHECK: Renderer value: ${cCGarEakAkZkDW.value}', tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]));
            S0jlNL.aNRrP(
              '🐥 TEXTURE CHECK: Video rotation: ${cCGarEakAkZkDW.value.rotation}',
              tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]),
            );
            ...
            S0jlNL.aNRrP('🐥 TEXTURE CHECK: Has valid texture ID: $hasValidTexture', tag: String.fromCharCodes(const <int>[87, 101, 98, 82, 84, 67]));
          });
        }
      });
    } else {
