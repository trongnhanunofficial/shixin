...
class WKIM {
  ...
  // Server device ID (from device() API response)
  // Used in clientMsgNo generation to match Android behavior
  ...
  /// This will cause received messages in this channel to use device timestamp instead of server timestamp
  /// This prevents message ordering issues when sending immediately after receiving a message
  void setActiveChatChannel(String channelId, int channelType) {
    connectionManager.setActiveChatChannel(channelId, channelType);
  }

  /// Clear the currently active chat channel
  /// This will cause all received messages to use server timestamp (default behavior)
  void clearActiveChatChannel() {
    connectionManager.clearActiveChatChannel();
  }

  /// Set server device ID (from GET /user/devices/{device_id} API response)
  /// This is used in clientMsgNo generation to match Android behavior
  /// Android: uuid + "_" + deviceId + "_1"
  /// Flutter: uuid + "_" + deviceId + "_5"
  void setServerDeviceID(String deviceID) {
    _serverDeviceID = deviceID.toString();
    if (kDebugMode) {
      // ignore: avoid_print
      print('🔑 WKIM: Server device ID set to: $_serverDeviceID');
    }
  }

  /// Get server device ID for clientMsgNo generation
  /// Returns "unknown" if not set (matches Android behavior)
  String getServerDeviceID() {
    if (_serverDeviceID == null || _serverDeviceID!.isEmpty) {
      return "unknown";
    }
