...
class WKMessageManager {
  ...
  HashMap<String, Function(WKMsg)>? _msgInsertedBack;
  ...
  /// Generate client message number matching Android pattern
  /// Android: uuid + "_" + deviceId + "_1"
  /// Flutter: uuid + "_" + deviceId + "_5" (5 indicates Flutter app)
  String generateClientMsgNo() {
    String deviceId = WKIM.shared.getServerDeviceID();
    String uuid = const Uuid().v4().toString().replaceAll("-", "");
    return "${uuid}_${deviceId}_5";
  }
  ...
  Future<bool> _saveSyncChannelMSGs(List<WKSyncMsg> list) async {
    ...
    if (msgList.isNotEmpty) {
      // IMPORTANT:
      // Use single-message insert for sync results so we can deduplicate by
      // server identifiers (message_id / message_seq) and merge fields.
      // Bulk insertMsgList() historically deduped only by client_msg_no and could
      // introduce duplicates when server sync returns a different client_msg_no.
      try {
        for (final m in msgList) {
          await MessageDB.shared.insert(m);
        }
      } catch (_) {
        isSuccess = false;
      }
...
//                oldestOrderSeq = 0;
  ...
  setOnMsgInserted(WKMsg wkMsg) {
    if (_msgInsertedBack != null) {
      _msgInsertedBack!.forEach((key, back) {
        back(wkMsg);
      });
    }
  }

  addOnMsgInsertedListener(String key, Function(WKMsg) insertListener) {
    _msgInsertedBack ??= HashMap();
    if (key != '') {
      _msgInsertedBack![key] = insertListener;
    }
  }

  removeOnMsgInsertedListener(String key) {
    if (_msgInsertedBack != null) {
      _msgInsertedBack!.remove(key);
    }
  }
  ...
  updateSendResult(
    ...
    if (wkMsg != null) {
      ...
      // Cleanup duplicates that may have been inserted before send-ack (sync/recv)
      if (reasonCode == WKSendMsgResult.sendSuccess) {
        final toDelete = <String>{};
        if (messageID.isNotEmpty && messageID != '0') {
          final dupById = await MessageDB.shared
              .queryClientMsgNosByMessageIdExcept(messageID, clientSeq);
          toDelete.addAll(dupById);
        }
        if (messageSeq != 0) {
          final dupBySeq = await MessageDB.shared
              .queryClientMsgNosByChannelAndMessageSeqExcept(
            wkMsg.channelID,
            wkMsg.channelType,
            messageSeq,
            clientSeq,
          );
          toDelete.addAll(dupBySeq);
        }
        for (final dup in toDelete) {
          if (dup.isEmpty) continue;
          await deleteWithClientMsgNo(dup);
        }
      }
    }
  }
  ...
  updateContent(String clientMsgNO, WKMessageContent messageContent,
    ...
    if (wkMsg != null) {
      ...
      if (isRefreshUI && result > 0) {
        final updated =
            await MessageDB.shared.queryWithClientMsgNo(clientMsgNO);
        if (updated != null) {
